...
class Detection:
    ...
    def __extract_ouput(self, preds: ndarray, image_shape: Tuple[int, int], input_shape: Tuple[int, int],
    ...
    def __call__(self, image: ndarray, width: int = 640, height: int = 640,
                 score: float = 0.1, nms: float = 0.0, confidence: float = 0.0) -> dict:
...
def post_detection(file: bytes = File(...)):
    ...
    results = {
        "boxes": [list(map(int, box)) for box in results["boxes"]],
        "confidences": [float(score) for score in results["confidences"]],
